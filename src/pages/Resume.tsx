...
const Resume = () => {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="max-w-5xl mx-auto px-4 space-y-10 py-12"
    >
      {/* Back to Home */}
      ...
      >
        ...
        <div className="w-full bg-surface rounded-2xl border border-border overflow-hidden shadow-xl">
          {/* Mobile Fallback */}
          <div className="md:hidden p-8 text-center bg-white flex flex-col items-center justify-center min-h-[300px]">
            <DocumentTextIcon className="w-16 h-16 text-gray-400 mx-auto mb-4" />
            <h4 className="text-lg font-bold text-gray-800 mb-2">Resume Preview</h4>
            <p className="text-sm text-gray-600 mb-6">
              PDF preview is unavailable on mobile browsers. Please download the file to view it.
            </p>
            <a
              href={resumePdfPath}
              download="Harsh_Dadiya_Resume.pdf"
              className="inline-flex items-center px-6 py-3 bg-accent hover:bg-accentHover text-white rounded-xl font-medium transition-colors shadow-sm"
            >
          ...
          {/* Desktop Iframe */}
          <div className="hidden md:block relative w-full h-[650px] md:h-[850px] bg-white">
            <iframe
              src={`${resumePdfPath}#toolbar=1`}
              title="Harsh Dadiya Resume PDF Preview"
              className="w-full h-full border-0"
            >
              {/* Fallback for desktop browsers without PDF support */}
